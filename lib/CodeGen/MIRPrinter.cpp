...
void MIRPrinter::convert(ModuleSlotTracker &MST,
  ...
  if (MFI.getSavePoint()) {
    ...
    MFI.getSavePoint()->printAsOperand(StrOS, /*PrintType=*/false);
  }
  if (MFI.getRestorePoint()) {
    raw_string_ostream StrOS(YamlMFI.RestorePoint.Value);
    MFI.getRestorePoint()->printAsOperand(StrOS, /*PrintType=*/false);
  }
}
...
void MIRPrinter::convert(ModuleSlotTracker &MST,
  ...
  for (const auto &Table : JTI.getJumpTables()) {
    ...
    for (const auto *MBB : Table.MBBs) {
      raw_string_ostream StrOS(Str);
      MBB->printAsOperand(StrOS, /*PrintType=*/false);
...
void MIPrinter::print(const MachineBasicBlock &MBB) {
  ...
  if ((!MBB.succ_empty() && !SimplifyMIR) || !canPredictProbs ||
    ...
    for (auto I = MBB.succ_begin(), E = MBB.succ_end(); I != E; ++I) {
      if (I != MBB.succ_begin())
        OS << ", ";
      (*I)->printAsOperand(OS, /*PrintType=*/false);
...
void MIPrinter::print(const MachineInstr &MI, unsigned OpIdx,
  ...
  case MachineOperand::MO_MachineBasicBlock:
    Op.getMBB()->printAsOperand(OS, /*PrintType=*/false);
    break;
