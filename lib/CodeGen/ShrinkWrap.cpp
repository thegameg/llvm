...
public:
  ...
  unsigned isSave(const MachineBasicBlock &MBB) {
    if (!Save) {
      if (&MBB == &MBB.getParent()->front())
        return getCurrentCSRs(nullptr).size();
      return 0;
    }
    return &MBB == Save ? getCurrentCSRs(nullptr).size() : 0;
  }
  unsigned isRestore(const MachineBasicBlock &MBB) {
    ...
    return &MBB == Restore ? getCurrentCSRs(nullptr).size() : 0;
  }
...
bool ShrinkWrap::isShrinkWrapEnabled(const MachineFunction &MF) {
  auto BecauseOf = [&](const MachineBasicBlock *MBB, const char *Title) {
    MachineOptimizationRemarkMissed R{"shrink-wrap", Title, {}, MBB};
    ORE->emit(R);
    return false;
  };
  ...
  for (const MachineBasicBlock &MBB : MF) {
    ...
    for (const MachineInstr &MI : MBB)
      if (MI.isIndirectBranch())
        return BecauseOf(&MBB, "SkippedIndirectBranches");
  }
  return true;
}
